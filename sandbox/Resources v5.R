...
OptimizeModel <- function(p, p.fixed, data, FUN, err = 'absolute', ...) {
  ...
  ModelError <- function(p.var, p.fixed, data, FUN, err, ...) {
    ...
    if (err == 'absolute')      return(sum(abs(Fitted - Target)))
    if (err == 'squared')       return(sum((Fitted - Target) ^ 2))
    if (err == 'percent-abs')   return(sum(abs(Fitted / Target - 1)))
    if (err == 'percent-sq')    return(sum((Fitted / Target - 1) ^ 2))
